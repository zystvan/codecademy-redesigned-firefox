(function forumUpdater(window) {
    ...
    function insertCRListElm() {
        ...
        CRListElm.setAttrbiute("id", "canned-responses-list");
        CRListElm.appendChild(btn);

        CRContainer.appendChild(CRListElm);

        for (var i = 0, len = Object.keys(cannedResponses).length; i < len; i++)
            insertNewCRInList(cannedResponses[i].name, cannedResponses[i].body);
    }
    ...
    function insertNewCRInList(name, body, id) {
        ...
        li.dataset.cannedresponseid = id;
    ...
    function newCannedResponse() {
        ...
        var id = Object.keys(cannedResponses).length;
        ...
        insertNewCRInList(cannedResponseName, cannedResponseText, id);
    }
    ...
    function deleteCannedResponse(node) {
        var li = node.parentNode,
            cannedResponseId = li.dataset.cannedresponseid;
    ...
    function runTheCannedResponseFunctions() {
        ...
        CRListElm.addEventListener("click", function(ev) {
            ...
            if (tgN === "LI")
                prefillWithCannedResponse(cannedResponses[node.getAttribute("data-canned-response-id")]["body"]);
            else if (tgN === "SPAN")
